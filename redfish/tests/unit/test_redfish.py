...
def test_redfish(connection_parameters_mock, logger_mock):
    ...
    with pytest.raises(exception.ConnectionFailureException) as e:
        redfish.standard.Systems('http://127.0.0.1:9000',
                                 connection_parameters_mock())
    assert 'Connection error' in str(e.value.message)
